  ...
  public function index(){
        if(isset($_GET['filter']) && ! empty($_GET['filter'])){ // Cek apakah user telah memilih filter dan klik tombol tampilkan
            ...
            if($filter == '1'){ // Jika filter nya 1 (per tanggal)
                ...
                $ket = 'Data Transaksi Tanggal '.date('d-m-y', strtotime($tgl_masuk));
                $url_cetak = 'transaksi/cetak?filter=1&tahun='.$tgl_masuk;
                $transaksi = $this->TransaksiModel->view_by_date($tgl_masuk); // Panggil fungsi view_by_date yang ada di TransaksiModel
